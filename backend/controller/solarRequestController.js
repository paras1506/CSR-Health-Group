...
exports.searchByVillage = async (req, res) => {
    try {
        ...
        if (!village) return res.status(400).json({ error: "Village name is required." });

        const results = await SolarRequest.find({ villageName: new RegExp(village, "i") }).select("-donors");
        res.json(results);
    } catch (error) {
...
exports.donorInterest = async (req, res) => {
    try {
        ...
        await SolarRequest.findByIdAndUpdate(requestId, {
            $addToSet: { donors: { donorId: req.user.userId, name: req.user.name, email: req.user.email, phone: req.user.phone } }
        });
...
exports.updateFulfillment = async (req, res) => {
    try {
        ...
        if (!requestId || fulfillmentPercentage == null) return res.status(400).json({ error: "Request ID and fulfillment percentage are required." });

        const request = await SolarRequest.findById(requestId);
        if (!request) return res.status(404).json({ error: "Request not found." });

        await SolarRequest.findByIdAndUpdate(requestId, { fulfillmentPercentage }, { new: true });
...
exports.getInterestedDonors = async (req, res) => {
    try {
        ...
        if (!departmentId) {
            return res.status(403).json({ error: "Access denied. You are not a Department Head." });
        }
        ...
        res.status(500).json({ error: "Failed to fetch donor details", details: error.message });
    }
};
